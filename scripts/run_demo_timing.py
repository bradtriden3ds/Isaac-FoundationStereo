...
if __name__=="__main__":
  ...
  parser.add_argument('--input_dir', required=True, type=str,
                      help='Directory containing subfolders (image_1, image_2, ...) each with left/right images')
  ...
  # Process each subdirectory containing a pair of images
  subdirs = [d for d in os.listdir(args.input_dir) if os.path.isdir(os.path.join(args.input_dir, d))]
  for subdir in sorted(subdirs):
      sub_path = os.path.join(args.input_dir, subdir)
      left_path = os.path.join(sub_path, f"{subdir}_left.png")
      right_path = os.path.join(sub_path, f"{subdir}_right.png")
      if not (os.path.isfile(left_path) and os.path.isfile(right_path)):
          logging.warning(f"Skipping {subdir}: missing left/right images")
          continue
      ...
      if args.get_pc:
        ...
        np.save(os.path.join(out_subdir, 'depth_meter.npy'), depth)

        # save to depth_map.png file in mm
        depth_mm = (depth * 1000.0).astype(np.uint16)
        image = Image.fromarray(depth_mm)

        image.save(os.path.join(out_subdir, 'depth_map.png'))

        # save visual image
        vis = vis_disparity(disp)
        vis = np.concatenate([img0_ori, vis], axis=1)
        imageio.imwrite(os.path.join(out_subdir, 'vis.png'), vis)
        logging.info(f"Output saved to {out_subdir}")
        ...
        o3d.io.write_point_cloud(os.path.join(out_subdir, 'cloud.ply'), pcd)
        logging.info(f"PCL saved to {out_subdir}")

        if args.denoise_cloud:
          logging.info("[Optional step] denoise point cloud...")
          cl, ind = pcd.remove_radius_outlier(nb_points=args.denoise_nb_points, radius=args.denoise_radius)
          inlier_cloud = pcd.select_by_index(ind)
          o3d.io.write_point_cloud(os.path.join(out_subdir, 'cloud_denoise.ply'), inlier_cloud)
          pcd = inlier_cloud
  ...
  print(f"Average time: {sum(timing)/len(timing)}")
  print(f"Max time: {max(timing)}")
  print(f"Min time: {min(timing)}")

  print(f"Average time (excluding 1st): {sum(timing[1:])/len(timing)}")
  print(f"Max time(excluding 1st): {max(timing[1:])}")
  print(f"Min time(excluding 1st): {min(timing[1:])}")